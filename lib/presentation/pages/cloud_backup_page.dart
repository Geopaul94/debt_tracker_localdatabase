...
class _CloudBackupPageState extends State<CloudBackupPage> {
  ...
  Future<void> _createBackup() async {
    ...
    if (!isPremium) {
      ...
      final adShown = await _showRewardedAdForBackup();
      if (!adShown) {
        _showErrorSnackBar('📺 Please watch the ad to create a backup');
        return;
      }
    }
  ...
  Future<bool> _showRewardedAdForBackup() async {
    try {
  ...
  Future<void> _restoreBackup(BackupInfo backup) async {
    ...
    if (!isPremium) {
      ...
      final adShown = await _showRewardedAdForBackup();
      if (!adShown) {
        _showErrorSnackBar('📺 Please watch the ad to restore backup');
        return;
      }
    }
              ...
              builder: (context, snapshot) {
                ...
                if (!isPremium) {
                  return Container(
                    ...
                    child: Row(
                      children: [
                        Icon(Icons.ads_click, color: Colors.orange[600]),
                        SizedBox(width: 8.w),
                        Expanded(
                          child: Text(
                            'Free users need to watch an ad for each backup/restore',
                            style: TextStyle(
                              fontSize: 12.sp,
